...
import { commercialOfferSchema, type CommercialOfferFormValues } from '@/lib/validations';
...
export default function CommercialOfferForm({
  ...
  const {
    ...
    reset,
  } = useForm<CommercialOfferFormValues>({
    resolver: zodResolver(commercialOfferSchema),
  });

  const onSubmit = async (form: CommercialOfferFormValues) => {
    try {
      const email =
      ...
      addToast({
        ...
        message: email
          ? 'Коммерческое предложение будет отправлено на ваш email'
          : 'Мы свяжемся с вами в ближайшее время',
      });

      trackFormSubmit('commercial_offer');
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <Textarea
        ...
        {...register('message')}
        hint="Укажите желаемую мощность, количество, особые требования"
      />
      <Button
        type="submit"
        variant="primary"
        ...
        leftIcon={<FileText className="h-[18px] w-[18px]" aria-hidden />}
      >
        Получить КП
      </Button>
